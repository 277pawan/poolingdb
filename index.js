...
const withdraw = async (accountid, amount) => {
  ...
  try {
    ...
    const result = await client.query(`select * from account where id=$1 for update`, [accountid]);
...
console.log("we got the access")
    ...
    await client.query(`update account set balance =balance - $1 where id=$2`, [amount, accountid]);
